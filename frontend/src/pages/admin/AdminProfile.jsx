...
const AdminProfile = () => {
  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/users/admin-profile', {
        ...
        setFormData({
          ...
          image: data.userImage ? `http://localhost:5000${data.userImage}` : '',
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        ...formData,
        image: updated.userImage ? `http://localhost:5000${updated.userImage}` : '',
      });
  ...
  //       image: adminData.userImage ? `http://localhost:5000${adminData.userImage}` : '',
  //     });
  //   }
