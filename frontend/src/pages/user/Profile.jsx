...
const Profile = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [userRes, blogsRes, catRes] = await Promise.all([
          ...
          axios.get('http://localhost:5000/api/users/my-blogs', { withCredentials: true }),
  ...
  const handleEditBlog = (blog) => {
    setEditingBlog({
      ...
      blogImage: null,
      currentImage: blog.blogImage ? `http://localhost:5000${blog.blogImage}` : blog.image
    });
  };
  ...
  const handleDeleteBlog = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/users/blogs/${blogToDelete.blogID}`, {
  ...
  const handleUpdateBlog = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/users/blogs/${editingBlog.blogID}`,
        formData,
        { withCredentials: true }
      );
      ...
      setEditingBlog(null);
      // Refresh blogs
      const res = await axios.get('http://localhost:5000/api/users/my-blogs', { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put('http://localhost:5000/api/users/update', data, {
        withCredentials: true,
          ...
          <CardContent className="p-8">
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <Avatar className="h-24 w-24">
                <AvatarImage
                  src={currentUser.userImage ? `http://localhost:5000${currentUser.userImage}` : `https://api.dicebear.com/7.x/avataaars/svg?seed=${currentUser.userName}`}
                />
